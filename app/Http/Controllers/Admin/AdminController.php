...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request){
        ...
        if(in_array("admin-users",$menu_write)){
            $data['admin_action_display'] = 1;
        }else{
            $data['admin_action_display'] = 0;
        }

        if(in_array("portal-users",$menu_write)){
            $data['user_action_display'] = 1;
        }else{
            $data['user_action_display'] = 0;
        }
    ...
    public function forgotPassword(Request $request){
        ...
        if(isset($result[0])){
            if(!($result[0]->type == 'user')){
                ...
                $request->session()->flash('errorMessage',"Reset password link has been sent on your registered email");
                return redirect('/admin');
            }else{
            ...
            return redirect()->back()->with('errorMessage',"User does not exists.");
        }
    }
